...
- 사용자 메시지 말미의 **[출력 템플릿]**은 이모지·굵게(**)·줄바꿈·번호까지 포함해 한 글자도 바꾸지 말고 그대로 출력한 뒤, 괄호 안 안내만 실제 요약으로 바꿉니다.
- 섹션 제목(예: **2. 주요 방법론**)을 불릿(-) 줄 안에 다시 쓰지 마세요. 같은 문장을 두 번 반복하지 마세요.
- <redacted_thinking> 등 사고 과정·내부 추론 태그는 출력하지 않습니다."""

DIRECT_ANSWER_RAG_DEPTH_SUFFIX = (
    "\n템플릿의 📄·🔗 줄과 **1.~3.** 제목 줄은 그대로 두고, 각 '-' 불릿 아래만 더 구체적으로(방법·실험·수치가 있으면 포함) 길게 채우세요."
)
...
문서에 제공된 '논문 제목'과 '출처(arXiv 등)'를 반드시 찾아 맨 위에 적으세요. 내용이 부족하면 초록(Abstract)을 바탕으로 추론해서 작성하세요. 똑같은 문장을 절대 반복하지 마세요.

**[출력 템플릿]**
📄 **논문 제목:** (여기에 논문 제목 작성)
🔗 **출처/저자:** (여기에 출처나 저자 작성, 없으면 생략)
