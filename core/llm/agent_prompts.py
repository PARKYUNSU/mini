...
사용자가 여러 논문을 요청했습니다.
🚨 **반드시 [참고 문서]에 제공된 모든 문서(최대 5개)를 빠짐없이 각각 요약하세요.**
문서가 주제와 간접적으로 관련 있어도 해당 논문이 주제를 어떻게 다루는지 요약해야 합니다.
제공되지 않은 논문을 지어내는 것만 금지합니다.

<b>📚 [주제어] 관련 최신 논문 서베이</b>

<b>1. 📄 (여기에 첫 번째 논문 제목)</b>
- 📅 발행일: (날짜) | 🔗 출처: (출처)
- 💡 핵심 요약: (이 논문이 어떤 문제를 어떻게 해결했는지 1~2줄로 요약)

<b>2. 📄 (여기에 두 번째 논문 제목)</b>
- 📅 발행일: (날짜) | 🔗 출처: (출처)
- 💡 핵심 요약: (이 논문이 어떤 문제를 어떻게 해결했는지 1~2줄로 요약)

(3번, 4번, 5번도 위와 동일한 양식으로 제공된 문서 개수만큼 반복)
"""
