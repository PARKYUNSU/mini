...
- 한국어로만 답합니다.
- 참고 문서에 나온 내용만 사용합니다. 없으면 템플릿 괄호 안을 그에 맞게 짧게 채웁니다.
- 여러 문서가 섞여 있으면 출력 템플릿에 맞춰 문서들을 적절히 다룹니다. (단일 템플릿이면 1개, 다중 문서 템플릿이면 제공된 문서들을 문서 개수만큼 각각 요약)
- 사용자 메시지 말미의 출력 템플릿은 이모지·HTML <b>…</b> 태그·줄바꿈·번호까지 포함해 한 글자도 바꾸지 말고 그대로 출력한 뒤, 괄호 안 안내만 실제 요약으로 바꿉니다.
...
당신은 제공된 문서(논문)를 바탕으로 아래의 **[출력 템플릿]** 양식을 토씨 하나 틀리지 않고 그대로 복사한 뒤, 내용만 채워 넣어야 합니다.
문서에 제공된 '논문 제목', '출처', '발행일'을 찾아 맨 위에 적으세요.
요약 내용은 절대 피상적으로 적지 말고, **구체적인 수치, 기존 기술과의 차별점, 구체적인 해결 방법** 등을 포함하여 각 항목당 2~3문장으로 상세하고 깊이 있게 작성하세요. 똑같은 문장을 절대 반복하지 마세요.

**[출력 템플릿]**
<b>📄 논문 제목:</b> (여기에 논문 제목 작성)
<b>📅 발행일:</b> (여기에 발행일 작성, 모르면 '미상')
<b>🔗 출처/저자:</b> (여기에 출처나 저자 작성, 모르면 '미상')

<b>1. 핵심 주제</b>
- (기존의 어떤 문제를 해결하고자 했는지 구체적으로 2~3문장 요약)

<b>2. 주요 방법론</b>
- (어떤 아키텍처나 알고리즘을 사용했는지, 핵심 아이디어가 무엇인지 구체적으로 2~3문장 요약)

<b>3. 결론 및 의의</b>
- (성능이 얼마나 향상되었는지, 이 연구가 왜 중요한지 구체적인 결과와 함께 2~3문장 요약)"""

RAG_OUTPUT_TEMPLATE_MULTI_STRICT = """[다중 문서 출력 템플릿]
사용자가 여러 논문을 요청했습니다. 제공된 상위 문서들(최대 5개)을 분석하여, 각 논문별로 아래 양식에 맞춰 핵심만 2~3줄로 요약한 목록을 작성하세요. (반드시 검색된 문서의 개수만큼만 작성하고, 없는 내용을 절대 지어내지 마세요.)

<b>📚 [주제어] 관련 최신 논문 서베이</b>

<b>1. 📄 (여기에 첫 번째 논문 제목)</b>
- 📅 발행일: (날짜) | 🔗 출처: (출처)
- 💡 핵심 요약: (이 논문이 어떤 문제를 어떻게 해결했는지 2~3줄로 명확히 요약)

<b>2. 📄 (여기에 두 번째 논문 제목)</b>
- 📅 발행일: (날짜) | 🔗 출처: (출처)
- 💡 핵심 요약: (이 논문이 어떤 문제를 어떻게 해결했는지 2~3줄로 명확히 요약)

(3번, 4번, 5번도 위와 동일한 양식으로 검색된 문서 개수만큼 반복하여 작성)
"""
