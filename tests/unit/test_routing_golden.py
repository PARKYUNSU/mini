...
def test_search_intent():
    from core.graph.agent_router_rules import get_search_intent

    cases_path = Path(__file__).resolve().parents[1] / "golden_routing_cases.yaml"
...
def test_whitelisted_tool():
    from core.graph.agent_router_rules import match_whitelisted_tool

    cases_path = Path(__file__).resolve().parents[1] / "golden_routing_cases.yaml"
    agent_tools_dir = Path(__file__).resolve().parents[2] / "tools" / "runtime" / "agent_tools" / "agent_tools"
    cases = _load_yaml(cases_path)
