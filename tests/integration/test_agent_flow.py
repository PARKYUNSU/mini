...
def run_ollama_check(*, verbose: bool) -> str:
...
def run_groq_check(*, verbose: bool) -> str:
...
def run_gemini_check(*, verbose: bool) -> str:
...
def run_chromadb_check(*, verbose: bool) -> str:
    print("3. ChromaDB (RAG) 테스트...")
    try:
        from core.rag.agent_chroma_rag import ChromaRAGTool
...
def run_e2b_check(*, verbose: bool) -> str:
...
def run_full_graph_smoke(*, verbose: bool, quiet_graph: bool) -> str:
...
def run_full_graph_code_run(*, verbose: bool, quiet_graph: bool) -> str:
...
def main() -> int:
    ...
    if run["ollama"]:
        tally(run_ollama_check(verbose=args.verbose))
    if run["groq"]:
        tally(run_groq_check(verbose=args.verbose))
    if run["gemini"]:
        tally(run_gemini_check(verbose=args.verbose))
    if run["chroma"]:
        tally(run_chromadb_check(verbose=args.verbose))
    if run["e2b"]:
        tally(run_e2b_check(verbose=args.verbose))
    if run["graph"]:
        tally(run_full_graph_smoke(verbose=args.verbose, quiet_graph=args.quiet_graph))
        tally(run_full_graph_code_run(verbose=args.verbose, quiet_graph=args.quiet_graph))
