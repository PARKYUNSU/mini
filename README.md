...
| 경로 | 역할 |
|------|------|
| `apps/` | 실행 진입점 (`telegram_bot`, `scheduler`, `backfill`) |
| `core/` | 그래프·LLM·RAG·세션·실행·설정 등 공유 도메인 로직 |
| `pipelines/` | 수집/백그라운드 파이프라인 (`ingest`, `debate`) |
| `tools/` | 런타임 도구(`runtime/agent_tools/…`), `cron_engine/` |
| `tests/` | `unit/`, `integration/`, `external/` |

프로젝트 루트(`mini/`)에서 실행할 때는 **`PYTHONPATH`에 루트가 포함**되어야 패키지 import가 동작합니다. `restart.sh`는 `PYTHONPATH`에 현재 디렉터리를 넣은 뒤 `python -m apps.telegram_bot.main` 등으로 기동합니다. `pytest`는 `pyproject.toml`의 `pythonpath = ["."]` 로 동일하게 맞춰 두었습니다.

---

## 1. 실행 순서

**Python 3.10+** 권장 (텔레그램 봇·LangGraph·타입 힌트). macOS 기본 `/usr/bin/python3`(3.9)은 부족할 수 있습니다.
...
python scripts/check_agent_env.py   # 필수 패키지 확인 (통과해야 봇·pytest 의미 있음)
...
**중요**: 수집 파이프라인(`pipelines.ingest.main`)을 먼저 실행해 `chroma_db/`와 `raw_data_queue/crawled_papers.jsonl`을 채워야 봇이 의미 있는 답변을 합니다.

### 과거 논문 대량 수집 (백필, 기간 기반)

일일 수집은 `pipelines.ingest.main`이 담당합니다. **기간을 지정**해 과거 논문을 일괄 수집하려면:

```bash
python -m apps.backfill.run_backfill                          # 기본: 2024-01-01 ~ 2024-12-31
python -m apps.backfill.run_backfill -s 2023-01-01 -e 2023-12-31   # 2023년 전체
python -m apps.backfill.run_backfill -s 2024-06-01 -e 2024-12-31 -b 20  # 20개씩 페이징
```
...
학습용 Q&A 생성은 `pipelines/debate/llm_debate_scheduler.py`가 전담합니다.
...
- **권장**: `processed/`를 통째로 되돌리기보다 **백필(또는 매일 수집 스케줄)**로 **새 논문**을 큐에 쌓습니다.
- **빠른 보충**: 최근 N일 구간만 백필하려면 `bash scripts/refill_crawled_queue.sh` (기본 N=90, `REFILL_QUEUE_LOOKBACK_DAYS`로 변경 가능). `apps.backfill.run_backfill` 인자만 쓰려면 `bash scripts/refill_crawled_queue.sh -- -s 2025-01-01 -e 2025-03-01`.

---

## 2. 환경 변수 (.env)

메인 봇(`apps.telegram_bot.main`) 기동 시 **Groq + Gemini 키(최소 1개)**·텔레그램이 모두 필요합니다.

| 변수 | 용도 | 필수 |
|------|------|------|
| `GROQ_API_KEY` | Agent **Executor / Monitor** (코딩·검수) | ✅ (메인 봇) |
| `GEMINI_API_KEY` | 라우터 폴백·도구·Tavily 요약·**LLM 토론(Gemini 비평)** 등 | ✅ (메인 봇, 키 1개 이상) |
| `GEMINI_API_KEYS` | 쉼표 구분 다중 키 — 있으면 이 목록만 사용 (`GEMINI_API_KEY` 단독 설정 무시) | 위와 동일 |
| `GEMINI_API_KEY_2` … `_20` | `GEMINI_API_KEYS`가 비어 있을 때 순서대로 합쳐서 사용 (429 시 순환) | 선택 |
| `TELEGRAM_TOKEN` | 메인 봇 | ✅ |
...
| `TUYA_*` | Tuya 스마트 플러그 (`tools/runtime/agent_tools/agent_tools/smart_plug.py`) — `TUYA_CONTROL_MODE=local`(기본) 또는 `cloud` | 플러그 제어·스케줄 실행 시 |

스마트 플러그 + cron 연동은 **`docs/SMART_PLUG_CRON.md`**, LAN이 막힌 기기는 **`docs/SMART_PLUG_CLOUD.md`** 참고.

---

## 3. 메인 봇 사용법 (`apps.telegram_bot.main`)
...
### LLM 토론 스케줄러 (`pipelines/debate/llm_debate_scheduler.py`)

- **통합 스케줄**에서 월~금 **02:00** 배치를 넣으려면 `.env`에 **`LLM_DEBATE_SCHEDULE_ENABLED=1`** (기본·미설정은 **등록 안 함**). 켜진 경우에만 `apps/scheduler/run_scheduler.py`가 `pipelines/debate/llm_debate_scheduler.py --test` 호출. **`LLM_DEBATE_BATCH_DURATION_SEC`** (기본 **0**): `0`이면 **시간 제한 없이 백그라운드 기동**만 하고 스케줄 메인 루프는 즉시 돌아옵니다(로그: `.cron/llm_debate_batch_stdout.log`). **실행 중인 토론 배치 PID는 `.cron/llm_debate_child.pid`에 기록되고, `.llm_debate.telegram.pid`에 동일 값이 미러됩니다**(`pipelines/debate/llm_debate_spawn_guard.py`). 장시간 실행 중에는 `.cron/llm_debate_health.json`에 heartbeat(최근 업데이트 시각/상태)를 남기며, 설정값(`LLM_DEBATE_HEARTBEAT_MAX_STALE_SEC`)을 초과해 갱신이 없으면 가드가 장애 상태로 보고 PID 파일을 정리한 뒤 다음 기동을 허용합니다. 스케줄·텔레그램 어느 쪽으로 띄웠든 **이미 `llm_debate_scheduler` 자식이 살아 있으면 중복 기동하지 않습니다.** 양수면 그만큼 초 동안 **동기** 실행(기간 동안 PID 파일 유지 후 종료 시 정리).
...
```bash
python pipelines/debate/llm_debate_scheduler.py           # 스케줄 대기
python pipelines/debate/llm_debate_scheduler.py --test    # 즉시 1회 (기본 시간 한도는 EVENT_DURATION_SEC, `--duration-sec 0` 이면 무제한)
python pipelines/debate/llm_debate_scheduler.py --test --file sample.jsonl --max-records 3
```
...
mini/
├── apps/
│   ├── telegram_bot/       # 메인 봇·수신기·워커 (`main.py`, `telegram_receiver.py`, …)
│   ├── scheduler/          # 통합 스케줄러·cron 워커
│   └── backfill/           # 기간 백필 CLI
├── core/                   # graph, llm, rag, session, execution, config, bridge
├── pipelines/
│   ├── ingest/             # arXiv 수집·파싱·RAG 적재 (구 `src/`)
│   └── debate/             # LLM 토론 스케줄러·스폰 가드
├── tools/
│   ├── runtime/agent_tools/  # 실행 도구 모듈 (`agent_tools/` 실파일은 하위 `agent_tools/`)
│   └── cron_engine/        # 텔레그램 스케줄 엔진
├── scripts/                # 점검·런처·기타 스크립트
├── tests/                  # unit / integration / external
├── pyproject.toml
├── restart.sh              # 봇 기동 (PYTHONPATH + `python -m apps.telegram_bot.*`)
├── agent_learnings/
├── chroma_db/
├── raw_data_queue/
└── finetune_datasets/
...
작업·시각·로그·Boardroom 환경변수 요약은 **[docs/SCHEDULER_AND_CRON.md](docs/SCHEDULER_AND_CRON.md)** 참고.

**`apps.scheduler.run_scheduler`가 꺼져 있으면** 월~금 02:00 LLM 토론·매일 arXiv 파이프라인·텔레그램 cron_engine **전부 동작하지 않습니다.** 반드시 상시 프로세스로 띄워 두세요.
...
source .venv/bin/activate
export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$(pwd)"
nohup python3 -u -m apps.scheduler.run_scheduler >> scheduler.log 2>&1 &
```
...
```bash
export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$(pwd)"
nohup python -u -m apps.telegram_bot.main >> agent.log 2>&1 &
tail -f scheduler.log
tail -f agent.log
```

- **arXiv 파이프라인**: 매일 **01:00** 실행 (기본, `.env`의 `ARXIV_PIPELINE_SCHEDULE_AT`). 메타 페이징·상한은 `ARXIV_INGEST_MAX_SCAN` 등 — [docs/SCHEDULER_AND_CRON.md](docs/SCHEDULER_AND_CRON.md).
...
- **실행 주체**: 통합 스케줄러(`apps.scheduler.run_scheduler`)가 떠 있어야 함. 메인 봇만 켜 두면 **등록만 되고 실행 루프는 돌지 않음**(단, `restart.sh` 분리 모드는 봇 프로세스 안에서 cron 워커를 띄우는 경로도 있음).
...
python -c "import apps.telegram_bot.main; import core.graph.agent_graph; print('import OK')"   # 모듈 스모크
...
| 수동 통합 점검 | `python tests/integration/test_agent_flow.py` |
| 그래프만·로그 줄이기 | `python tests/integration/test_agent_flow.py --only graph --quiet-graph` |
...
- **봇이 새로 저장하는 도구**는 `tools/runtime/agent_tools/agent_tools/saved/` 아래에만 쓰며, 이 디렉터리는 `.gitignore` 처리됩니다. 레포에 포함할 **코어 도구**는 `tools/runtime/agent_tools/agent_tools/*.py`에 둡니다.

- **`tests/integration/test_agent_flow.py`** (스크립트, pytest 아님): 외부 서비스 점검. **환경이 없으면 FAIL 대신 SKIP**으로 표시. 시작 시 환경 요약 출력. 옵션: `--only all|ollama|groq|gemini|chroma|e2b|graph` (기본 `all`), `--verbose` (traceback 전체), `--quiet-graph` (5a/5b DEBUG print 억제). 종료 코드는 **FAIL이 하나라도 있을 때만 1**. E2B `실행 오류` 문자열은 FAIL. 5b에서 executor까지 갔으나 샌드박스만 실패하면 **SKIP/WARN** 처리(라우팅은 `tests/test_code_run_state.py`로 검증).
- **`pyproject.toml`**: `pytest` 마커 `unit` / `integration` / `external` 정의.
- **테스트 계층 (요약)**  
  - **A · unit**: `agent_router_rules`, 골든 라우팅, `test_code_run_state`, E2B env 구성 등.  
  - **B · integration**: `tests/unit/test_node_contracts.py`(router/direct_answer/executor/monitor mock), `tests/integration/test_graph_smoke.py`.  
  - **C · external**: `tests/external/test_external_integration.py` — 로컬에서 키·Ollama 있을 때만 의미 있음.  
  - **D · 수동**: `tests/integration/test_agent_flow.py` 스크립트(SKIP/FAIL 구분).
...
**라우터 단위 테스트 (pytest 없이):** `core.graph.agent_router_rules`는 (설계상) chromadb 없이도 import 가능한 경로를 유지합니다.

```bash
python tests/unit/test_intentional_syntax_and_router.py
python tests/unit/test_routing_golden.py
```
