...
_MINI = Path(__file__).resolve().parent
if str(_MINI) not in sys.path:
    sys.path.insert(0, str(_MINI))

from agent_bridge_queue import (
    bump_chat_thread,
...
from paper_list_light import list_stored_papers_text as papers_list_stdlib
    ...
    try:
        subprocess.run(
            [
                sys.executable,
                "-c",
                f"from agent_session import clear_session; clear_session({chat_id!r})",
            ],
...
def main() -> None:
    ...
    def on_reboot(message):
        ...
        try:
            subprocess.Popen(["bash", str(_MINI / "restart.sh")], cwd=str(_MINI), start_new_session=True)
        except Exception as e:
    ...
    def on_backfill_start(message):
        ...
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from agent_backfill_telegram import start_backfill_process; start_backfill_process()",
            ],
            cwd=str(PROJECT_ROOT),
    ...
    def on_backfill_stop(message):
        ...
        subprocess.run(
            [
                sys.executable,
                "-c",
                "from agent_backfill_telegram import stop_backfill_process; stop_backfill_process()",
            ],
            cwd=str(PROJECT_ROOT),
            timeout=120,
        )
    ...
    def on_debate_start(message):
        ...
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from agent_debate_telegram import start_llm_debate_telegram_process; start_llm_debate_telegram_process()",
            ],
            cwd=str(PROJECT_ROOT),
            start_new_session=True,
        )
    ...
    def on_debate_stop(message):
        ...
        subprocess.run(
            [
                sys.executable,
                "-c",
                "from agent_debate_telegram import stop_llm_debate_telegram_process; stop_llm_debate_telegram_process()",
            ],
            cwd=str(PROJECT_ROOT),
            timeout=120,
        )
    ...
    def handle(message):
        ...
        if cmd == "/paper" or cmd.startswith("/paper@"):
            subprocess.run(
                [sys.executable, str(_MINI / "paper_mode_cli.py"), chat_id, text],
                cwd=str(PROJECT_ROOT),
                timeout=60,
            )
