...
2. **`run_scheduler.py`가 떠 있어야 함**  
   - 1분마다 due job을 찾아 `agent_scheduled_runner.run_scheduled_job(prompt, chat_id)` 호출.  
   - `is_scheduled=True`일 때도 **라우터 화이트리스트**가 `smart_plug`로내면 Planner 없이 바로 도구 실행.
...
- 스케줄이 안 돌면 **`run_scheduler.py` 프로세스**가 살아 있는지 확인 (`ps aux | grep run_scheduler`).  
